...
所有本地命令一律 `/` 开头（全角 `／` 同样认），裸词一概当普通聊天。发 `/帮助` 拿按自己权限过滤后的命令表。

命中的命令在适配器本地处理并直接回复，不进入模型上下文、不烧 token（`/生图` 例外，它是钉住绘图节点后继续走模型）。

群里想不 @Bot 直接发命令，需要开 `trigger.signals.prefix`；没开时先 @Bot 再写命令一样有效，私聊不受影响。命令首段里再出现斜杠的不算命令，群里贴 `/var/log/x` 这类路径不会被误判。

唯一免斜杠的是「引用审批卡片后回一个动词」——卡片在，指向唯一，不存在误触发。
...
| `/命名表情 3 开心` | 仅管理员 | 给已有收藏表情设置/修改描述；第一个参数可用序号、md5、resId、文件名或旧描述；成功后会同步名称文件 |
...
检测到联网搜索工具进度时，会向会话发送低频文本提示，例如“已收到联网搜索请求，正在检索网页资料……”，避免用户误以为 Bot 卡住。

模型可在回复里写 `[REACT:ID]` 给触发消息贴一个表情表态。该通路受 `ONEBOT_ENABLE_REACTIONS` 开关与内置白名单双重约束：开关关闭时完全静默，白名单外的 ID 直接丢弃。可用 ID 由入站提示词【QQ表情表态】动态列出。一条触发消息最多贴一个 —— 模型可能一次写多个标记，中间回复与最终回复也会各解析一次，数量由代码兜底而不是靠提示词自觉。
