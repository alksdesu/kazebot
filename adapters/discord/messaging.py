...
Message output and file handling share DiscordRuntime explicitly. Does not
import context.py, keeping the import direction one-way.
"""
...
# 只保留下游能编码的图片类型；svg/tiff/ico/avif 会被 _guess_mime_from_path 兜底成 image/png，收下只是把失败推后。
...
# 读之前先看 att.size，避免把超大文件整个读进内存。
_ATTACHMENT_MAX_BYTES = int(os.environ.get("DISCORD_ATTACHMENT_MAX_BYTES", str(50 * 1024 * 1024)))
...
async def _collect_attachments(
    ...
    for i, att in enumerate(all_atts):
        ...
        if att_size > _ATTACHMENT_MAX_BYTES:
            logger.warning("  att[%d]: too large (%s bytes), skipped %s", i, att_size, getattr(att, 'filename', '?'))
            continue
        try:
            ...
            if att_type == "image":
                ...
                if not sniffed:
                    logger.warning("  att[%d]: unsupported image header, skipped %s", i, getattr(att, 'filename', '?'))
                    continue
                mime = sniffed
...
def _record_bot_reply(rt: Any, channel_id: int, text: str, *, msg_id: int | None = None) -> None:
    ...
    if len(q) > rt.history_max_len:
        history.note_gap(rt, channel_id, int(q.pop(0).get("seq", -1)))
