...
Owns settings, DiscordRuntime, event registration, SDK initialization, and the
top-level Discord client startup.
"""
...
class DiscordRuntime:
    ...
    # 队列满时被丢掉、且当时还没送到 Clonoth 的最高序号，按频道记。
    history_gap: dict[int, int] = field(default_factory=dict)
...
async def _initialize_sdk(rt: DiscordRuntime) -> None:
    ...
    # 同一个 env 只能有一个默认：另写一个绝对路径会让没配 env 的部署去读一个不存在的 token。
