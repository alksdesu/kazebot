...
| `DISCORD_BRIDGE_HOST` | 否 | `127.0.0.1` | 工具侧连 Bridge 的地址（`tools/discord_manage.py` 读） |
| `DISCORD_ENTRY_NODE` | 否 | `main` | 入口节点 ID |
| `DISCORD_NODE_NAMES` | 否 | `{}` | 节点显示名映射，JSON 格式 |
| `DISCORD_HISTORY_LEN` | 否 | `15` | 每个频道保留的历史消息条数 |
| `DISCORD_ATTACHMENT_MAX_BYTES` | 否 | `52428800`（50 MiB） | 单个附件下载上限。先看 `attachment.size` 再决定读不读，避免把超大文件整个读进内存 |
...
ExecStart=/usr/local/bin/python3.11 -m adapters.discord.app
Restart=always
RestartSec=5
