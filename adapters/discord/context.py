...
Identity cleanup, history storage, reaction bookkeeping, and reply context
extraction all take the shared DiscordRuntime instead of module-level globals.
"""
...
def _build_context_text(
    ...
    *,
    ...
    lost_lines = history.lost(rt, channel_id, wm)
    block_lines: list[str] = []
    if lost_lines > 0:
        # 不说这一句，模型会把中间断掉的一段聊天当连续的推理。
        block_lines.append(f"（此前 {lost_lines} 条消息超出缓存上限，未包含在内）")
    if history_entries:
        block_lines.extend(_fmt_entry(e) for e in history_entries)
    elif lost_lines <= 0:
        block_lines.append("（无新消息）" if wm >= 0 else "（暂无历史）")
    history_block = "\n".join(block_lines)
