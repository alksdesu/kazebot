...
import { SETTINGS_GROUP_LABELS, SETTINGS_GROUP_ORDER, settingsTabs } from './settingsTabs';
    ...
    <div className="flex h-full min-h-0 flex-col">
      ...
      <nav aria-label="设置分区" className="min-h-0 flex-1 overflow-y-auto p-2">
        {SETTINGS_GROUP_ORDER.map((group) => {
          const tabs = settingsTabs.filter((tab) => (tab.group || 'basics') === group);
          if (tabs.length === 0) return null;
          const heading = SETTINGS_GROUP_LABELS[group];
          return (
            <div className="mb-2" key={group}>
              {heading && (
                <p className="px-3 pb-1 pt-2 font-mono text-[0.55rem] uppercase tracking-[0.18em] text-[var(--duties-tertiary)]">
                  {heading}
                </p>
              )}
              {tabs.map((tab) => {
                ...
                return (
                  <button
                    ...
                    // 可及名称里不带图标：图标是给眼睛扫的，读屏和测试要认稳定的文字。
