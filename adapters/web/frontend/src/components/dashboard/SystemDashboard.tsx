...
import { countActiveTasks, formatUptime } from '../../utils/systemStatus';
...
const Stat = ({ label, value, detail, onClick }: { label: string; value: string | number; detail?: string; onClick?: () => void }) => {
  ...
  const cls = `border border-[var(--duties-border)] bg-[var(--duties-bg)] p-2.5 ${onClick ? 'w-full cursor-pointer text-left transition-colors hover:bg-[var(--duties-muted)]' : ''}`;
  if (onClick) {
...
export const SystemDashboard = () => {
  ...
  const activeTasks = countActiveTasks(data.adminState?.tasks);
