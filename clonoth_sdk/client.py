...
from .types import Event, HealthInfo, InboundResult, IntentVerdict, OpenAIConfig, RunningTask
...
class ClonothClient:
    ...
    async def submit_inbound(
        ...
        *,
        ...
        Args:
            ...
            entry_node_pinned: 入口节点由用户命令选定，不接受 Supervisor 侧能力路由改写。
            memory_hints: 本轮涉及的人（{"subjects": [别名]}），用于按人加载长期记忆。
                单独一个参数而不塞进 platform_auth，是因为后者参与鉴权判定，
                混入非鉴权字段会让 is_admin 的判断面变大。
        ...
        if entry_node_id:
            ...
            payload["entry_node_pinned"] = bool(entry_node_pinned)
        if platform_auth:
            payload["platform_auth"] = dict(platform_auth)
        if route_hints:
            payload["route_hints"] = dict(route_hints)
        if memory_hints:
            payload["memory_hints"] = dict(memory_hints)
    ...
    async def judge_qq_intent(
        self,
        *,
        conversation_key: str,
        text: str,
        channel: str = "qq_group",
        ...
        """问一次模型：这条群消息是不是在跟 bot 说话。

        对应 POST /v1/qq/intent/judge。任何失败都返回 decided=False / agreed=False，
        不抛异常 —— 判定层出问题时群消息必须照常放行，不能反过来拖住 bot。

        Args:
            context_lines: 最近几条群聊，必须是已匿名化的文本行。
            bot_names: bot 在群里被叫的名字，判定「有没有在点名」要用。
            timeout_sec: 等模型的上限。HTTP 侧多给 2 秒，让服务端先超时并取消任务。
        """
        payload: dict[str, Any] = {
            "conversation_key": conversation_key,
            "text": text,
        ...
        if context_lines:
            payload["context_lines"] = list(context_lines)
        if bot_names:
            payload["bot_names"] = list(bot_names)
        if speaker:
        ...
        try:
            resp = await self._http().post(
                f"{self._base_url}/v1/qq/intent/judge",
                json=payload,
            ...
            return IntentVerdict(error=str(exc)[:200])
        if not isinstance(data, dict):
            return IntentVerdict(error="malformed response")
