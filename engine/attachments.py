...
def sniff_image_mime(content: bytes) -> str:
    """Return the image MIME implied by the file header, or "" when it is not a supported image."""
    # Magic numbers only: a mislabeled upload (PDF sent as an image segment) must not reach
    # the provider as base64 image data.
...
def _looks_like_svg(data: bytes) -> bool:
    """SVG 是文本，sniff_image_mime 认不出它。

    那个函数不该认：它的调用方靠「认不出就拒收」把非图片挡在附件之外，
...
def _resolve_file_url(url: str, workspace_root: Path) -> str | None:
    ...
    # 文件头优先：附件名是用户给的，.png 里装着别的东西是常事。认不出才退回扩展名。
    sniffed = sniff_image_mime(data) or ("image/svg+xml" if _looks_like_svg(data) else "")
    mime = sniffed or _guess_mime_from_path(rel_path)

    if mime in _UNSENDABLE_IMAGE_MIMES:
        _logger.warning("image format no provider accepts, skipping: %s (%s)", rel_path, mime)
        return None
