...
def _create_provider_from_registry(
    ...
    *,
    ...
    if provider_cls is None:
        # 保持回退到 OpenAI 的旧语义，但要留声：拼错块名时这里会拿着别家的 url+key
        # 发 OpenAI 格式的请求，静默的话只能看到一个没头没尾的 400。
        print(
            f"[engine] provider {requested_name!r} 未注册，回退成 openai 格式发送"
            f"（已注册：{','.join(sorted(provider_registry.list()))}）",
            flush=True,
        )
    ...
    for nodes_dir in nodes_dirs:
        ...
        for f in sorted(nodes_dir.iterdir()):
            ...
            if n is None or n.type != "ai" or n.id in seen_ids:
                continue
            # internal 节点不进枚举：它们回的是给程序看的判定值，被 AI switch 过去
            # 就会有人在群里收到一句 "no|群友之间在对话"。
            if bool((n.extra or {}).get("internal")):
                continue
            seen_ids.add(n.id)
...
async def _handle_task(
    ...
    main = normalize_openai_secret(cfg_raw)
    ...
    try:
        if kind == "node":
            result = await _run_node_task(
                ...
                api_key=main.api_key, base_url=main.base_url,
                default_model=main.model, default_provider=main.provider,
            )
        elif kind == "tool":
...
# 节点 id 与系统模型槽位的历史绑定。这两个节点 id 是内建常量，没有配置项能改。
_LEGACY_SYSTEM_MODEL_SLOTS = {
    "system.compactor": "compact",
    "system.turn_summarizer": "summary",
}


def _resolve_system_model_slot(node: Any) -> str:
    """节点用哪个 system_models 槽位。

    优先读节点 yaml 的 system_model_slot：意愿判断节点的 id 由 qq.yaml 配置，写死
    id→slot 映射的话，运营者改个节点名就静默丢掉独立渠道、回落主渠道模型。
    """
    declared = str((getattr(node, "extra", None) or {}).get("system_model_slot") or "").strip().lower()
    if declared:
        return declared
...
def _apply_system_slot(rp: Any, slot: Any, slot_name: str) -> None:
    ...
    if slot.provider:
        ...
        print(
            f"[engine] system slot {slot_name} 换了 base_url 但没指定 provider，"
            f"仍按 {rp.provider_type or 'openai'} 的格式发",
            flush=True,
        )
...
async def _run_node_task(
    *,
    ...
    rp = resolve_provider(
        ws_root, node, default_model,
        provider_default_type=default_provider,
        session_override=session_provider_override,
    )
...
async def _run_tool_task(
    *,
    ...
    kctx = ToolContext(
        ...
        # and leave node_id as the tool name. Purpose: approval events remain
        # traceable even outside native LLM tool-call execution.
