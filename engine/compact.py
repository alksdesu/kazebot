...
# ---------------------------------------------------------------------------
#  Token 估算校准
#  真实 token 数只在供应商响应里；预算裁剪只能按字符估。除数 3 是按英文定的，
#  中文约 1~1.5 字符/token —— 照 3 算等于把预算放大两三倍，压完仍超阈值，
#  下一轮真实值报回来又压一次。有真实值时就用它反推本会话的实际比例。
# ---------------------------------------------------------------------------

DEFAULT_CHARS_PER_TOKEN = 3.0
_CHARS_PER_TOKEN_MIN = 1.0
_CHARS_PER_TOKEN_MAX = 6.0


def observed_chars_per_token(total_chars: int, prompt_tokens: int | None) -> float:
    """用上一轮的真实 token 数校准字符/token 比；缺数据时回落默认值。

    夹在 [1, 6] 两头：图片 token 算进 prompt_tokens 但字符数里没有，比例会偏小；
    缓存命中让上报值异常小的话比例会暴涨。字符数不可能少于 token 数，1 是物理下限。
    """
    if not prompt_tokens or prompt_tokens <= 0 or total_chars <= 0:
        return DEFAULT_CHARS_PER_TOKEN
    return min(max(total_chars / prompt_tokens, _CHARS_PER_TOKEN_MIN), _CHARS_PER_TOKEN_MAX)


def total_content_chars(messages: list[Any]) -> int:
    """统计消息集的文本字符总数，多模态只数 text 块。"""
    total = 0
...
def should_compact(
    ...
    有 *last_prompt_tokens*（上一轮响应的 usage）就直接用，否则按字符数估算。
    """
    if threshold_tokens <= 0:
        return False
    if last_prompt_tokens is not None:
        return last_prompt_tokens > threshold_tokens
    divisor = chars_per_token if chars_per_token > 0 else DEFAULT_CHARS_PER_TOKEN
    return total_content_chars(messages) / divisor > threshold_tokens
...
def estimate_message_tokens(message: Any, chars_per_token: float = DEFAULT_CHARS_PER_TOKEN) -> int:
    """Estimate one message's token cost from its character count."""
    divisor = chars_per_token if chars_per_token > 0 else DEFAULT_CHARS_PER_TOKEN
    return int(total_content_chars([message]) / divisor)


def segments_within_token_budget(
    segments: list[list[Any]],
    keep_recent_tokens: int,
    chars_per_token: float = DEFAULT_CHARS_PER_TOKEN,
) -> int:
    """Return how many trailing segments fit into *keep_recent_tokens*.

    从最后一段往前累加，装不下就停。切在段边界上而不是消息上：一个 assistant 的
    tool_call 和它的 tool_result 永远在同一段里，按 token 逐条切会把它们拆开，
    供应商会拒。下限是 1 段 —— 当前活跃 task 不能被压掉，哪怕它自己就超预算。
    """
    if keep_recent_tokens <= 0 or not segments:
        return 0
    used = 0
    kept = 0
    for segment in reversed(segments):
        segment_tokens = sum(estimate_message_tokens(m, chars_per_token) for m in segment)
        if used + segment_tokens > keep_recent_tokens:
            break
        used += segment_tokens
        kept += 1
    return max(kept, 1)
...
def apply_compact_summary(
    ...
    *keep_recent_tokens* > 0 时保留量按 token 算，keep_recent 的段数语义不参与；
    为 0 时沿用按段数保留的旧行为。
    ...
    *keep_recent* until the result fits within the budget or keep_recent
    ...
    if threshold_tokens > 0 and keep_recent > 0:
        while should_compact(result, threshold_tokens, chars_per_token=chars_per_token) and keep_recent > 0:
            keep_recent -= 1
            if keep_recent > 0:
                kept_segments = segments[-keep_recent:]
            else:
