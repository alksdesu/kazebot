...
from .options import OptionSpec, catalog


# 这些是 provider 共用的基础设施，不是 provider 本身，扫描时跳过。
_NON_PROVIDER_MODULES = frozenset({"__init__", "base", "options", "degrade"})
...
class ProviderRegistry:
    ...
    @staticmethod
    def _wire_format(cls: type[BaseProvider]) -> str:
        return (getattr(cls, "wire_format", "") or getattr(cls, "provider_name", "")).strip().lower()

    def wire_formats(self) -> dict[str, str]:
        """每个 provider 的请求格式族。同族之间只换地址不会挂。"""
        return {name: self._wire_format(cls) for name, cls in sorted(self._registry.items())}

    def host_profiles(self) -> dict[str, list[str]]:
        """官方域名 → 收得下它的请求格式族。控制台照这份提示「地址和渠道对不上」。

        只收官方域名。中转站的域名看不出出身，认不出就闭嘴 —— 拦错比放过更烦人。
        取 __dict__ 而不是 getattr：继承来的域名不算数。
        """
        out: dict[str, set[str]] = {}
        for cls in self._registry.values():
            for host in cls.__dict__.get("official_hosts", ()):
                key = str(host).strip().lower()
                if key:
                    out.setdefault(key, set()).add(self._wire_format(cls))
        return {host: sorted(formats) for host, formats in sorted(out.items())}

    def default_vision_support(self) -> dict[str, bool]:
        """每家的模型默认收不收图片。渠道块显式写了 supports_vision 就不看这个。"""
        return {
            name: bool(getattr(cls, "default_supports_vision", True))
            for name, cls in sorted(self._registry.items())
        }

    def options_catalog(self) -> dict[str, list[dict]]:
        """每个 provider 支持的可调参数。控制台照这份清单渲染表单，不自己硬编码。"""
        return {
            name: catalog(getattr(cls, "OPTIONS", ()))
            for name, cls in sorted(self._registry.items())
        }
...
def auto_discover() -> ProviderRegistry:
    ...
    for module_info in pkgutil.iter_modules([str(package_dir)]):
        if module_info.ispkg or module_info.name in _NON_PROVIDER_MODULES:
            continue
