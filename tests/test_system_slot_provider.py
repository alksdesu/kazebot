...
class TestApplySystemSlot:
    def test_an_empty_slot_changes_nothing(self) -> None:
        rp = _main()
        _apply_system_slot(rp, SystemModel(), "compact")
        assert (rp.model, rp.provider_type, rp.api_key, rp.base_url) == (
            "main-model", "openai", "sk-main", "https://main.example/v1",
        )

    def test_the_slot_provider_wins(self) -> None:
        rp = _main()
        _apply_system_slot(
            rp,
    ...
    def test_model_only_slot_keeps_the_main_channel_format(self) -> None:
        # 最常见的用法：只换个便宜模型，url/key/格式都跟着主渠道。
        rp = _main()
        _apply_system_slot(rp, SystemModel(model="cheap-mini"), "summary")
        assert rp.model == "cheap-mini"
        assert rp.provider_type == "openai"
        assert rp.base_url == "https://main.example/v1"

    def test_a_new_base_url_without_provider_is_reported(self, capsys) -> None:
        rp = _main()
        _apply_system_slot(rp, SystemModel(base_url="https://api.anthropic.com"), "intent")
        assert rp.provider_type == "openai"
        out = capsys.readouterr().out
        assert "intent" in out
        assert "provider" in out

    def test_switching_provider_alone_is_not_reported(self, capsys) -> None:
        rp = _main()
        _apply_system_slot(rp, SystemModel(provider="deepseek"), "compact")
